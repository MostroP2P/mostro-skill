#!/usr/bin/env tsx
/**
 * Dispute — Open a dispute on an active trade
 *
 * Usage:
 *   tsx scripts/dispute.ts --order-id <uuid>
 */
...
function parseArgs() {
  ...
  if (!opts.order_id) {
    console.error("Usage: dispute.ts --order-id <uuid>");
    process.exit(1);
  }
...
async function main() {
  ...
  console.log(`⚠️  Opening dispute for order: ${opts.orderId}`);
  console.log("   This will notify both parties and Mostro admins.\n");

  const message = buildOrderMessage("dispute", opts.orderId, requestId);
    ...
    for (const resp of filtered) {
      ...
      switch (kind.action) {
        case "dispute-initiated-by-you": {
          ...
          if (payload?.dispute) {
            console.log(`   Dispute ID: ${payload.dispute}`);
          }
          console.log("   An admin will review and resolve the dispute.");
          break;
        }
        case "cant-do":
          console.error(`❌ Cannot dispute: ${JSON.stringify(kind.payload)}`);
          break;
        default:
